...
Default username is **`admin`**. Change the password after first login (or rotate via Secrets Manager ΓÇö see `docs/runbook.md`).
...
2. Go to **Administration ΓåÆ Users and access ΓåÆ Users** (menu labels vary slightly by Grafana version).
3. **New user** or **Invite**:
   - Set login, name, and email.
   - Assign role:
     - **Viewer** ΓÇö view dashboards only
     - **Editor** ΓÇö create and edit dashboards
     - **Admin** ΓÇö manage users and settings within the organization
